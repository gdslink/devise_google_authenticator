  ...
  module CheckGA
    ...
    define_method :displayqr_resource_path_name do |resource, id|
      ...
      define_method :create do
        ...
        if resource.respond_to?(:get_qr) and resource.gauth_enabled? and resource.require_token?(cookies.signed[:gauth]) and !resource.class.is_using_saml #Therefore we can quiz for a QR
        ...
        elsif resource.respond_to?(:get_qr) and resource.gauth_enabled.to_i == 0 and resource.gauth_secret and !resource.class.is_using_saml
          #we head back into the checkga controller with the temporary id
